        ...
        {programs.map((program) => (
          <Card key={program.id} className="flex items-start justify-between gap-4 p-4">
            ...
            <div className="flex shrink-0 gap-2">
              <MotionButton variant="outline" size="icon-sm" onClick={() => startEdit(program)} aria-label="Edit">
                <Pencil className="h-3.5 w-3.5" />
              </MotionButton>
              <MotionButton variant="outline" size="icon-sm" onClick={() => handleDelete(program.id)} aria-label="Delete">
